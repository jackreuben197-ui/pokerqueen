...
export default class MttPayforHome extends BaseForm {
    ...
    protected lateLoad(): void {
        ...
        this.Gold_Type_Label = this.getChildNodeOrComponent(
            "Gold_Type_Label",
            cc.Label,
        );
    }
    async onShow(param?: any, fromUI?: cc.Node, sceneUI?: cc.Node) {
        ...
        this.setText(
            this.title_lbl,
            this._type == 1
                ? "UIMTTSignDialogBuyTitle"
                : "UIMTTSignDialogReBuyTitle",
        );

        this.Gold_Type_Label.string = UIMTTModel.Instance.GetGoldTypeName(
            UIMTTModel.Instance.MttInfo.mtt.gold_type,
        );
        ...
        if (this._data.gold_type == 4) {
            ...
            await UIClubModel.mInstance.WebMttUserWallet(
                GC.data.mtt.list.select.match_id,
                { club_id: ClubCache.club_id, offset: 0, limit: 20 },
            );
            if (this._type == 1) {
                ClubCache.mttPayWallat = null;
            } else {
                ClubCache.mttPayWallat = ClubCache.mttPayWallat
                    ? ClubCache.mttPayWallat
            ...
            if (UIMTTModel.Instance.MttInfo.mtt.buy_prop_id != 0) {
                UIMTTModel.Instance.APIPropUserCheckPropInfo((res) => {
                    if (res.code == 0) {
                        this.cachePropPropertyType =
                            res.data.prop_property_type;
                        this.cachePropBalance = res.data.prop_balance;
                        this.cacheIsFreeServiceFee =
                            res.data.is_free_service_charge;
    ...
    initSelectWallet() {
        if (ClubCache.mttPayWallat == null) {
            ...
            _data.wallet.some((element) => {
                walletData = element;
                return (
                    element.club_random_id ==
                    ClubCache.mttPayWallat.club_random_id
                );
            });
            ...
            if (this.totalRebuyTimes < 10000) {
                ...
                if (UIMTTModel.Instance.MttInfo.state != null) {
                    num2.string =
                        UIMTTModel.Instance.MttInfo.state.left_rebuy_times.toString();
                } else {
    ...
    HandleDate() {
        ...
        if (this._data.rebuyData != null) {
            let deadLineTime =
                TimeHelper.RFC3339TimeConvertToUTCTime(
                    this._data.rebuyData.starTime,
                ) +
                (this._data.rebuyData.rebuyBlind - 1) *
                    this._data.rebuyData.upblindInterval;
            this.rebuySecond = deadLineTime - TimeHelper.Now / 10000000;
            if (this.rebuySecond > 15) {
                this.rebuySecond = 14;
                UIComponent.Instance.Toast(
                    CPErrorCode.LanguageDescription(10012) + "(15s)",
                );
                ...
                if (this.rebuySecond < 0) {
                    UIComponent.Instance.Toast(
                        CPErrorCode.LanguageDescription(10012) + "(" + 0 + "s)",
                    );
                    // this.textCommit.string = CPErrorCode.LanguageDescription(10012) + "(" + 0 + "s)";
                } else {
                    UIComponent.Instance.Toast(
                        CPErrorCode.LanguageDescription(10012) +
                            "(" +
                            this.rebuySecond +
                            "s)",
                    );
    ...
    UpdateGold(discount = 0, DiscountType = 0) {
        ...
        if (this.isUseLimitFree) {
            switch (this._data.mTTJoinMode) {
                ...
                case MTTJoinMode.Apply:
                    if (
                        this._data.buyin_free_incl_svr == 0
                    ) //限免是否包含服务费，0不包含，1包含
                    {
                ...
                case MTTJoinMode.Rebuy:
                    if (
                        this._data.rebuy_free_incl_svr == 0
                    ) //限免是否包含服务费，0不包含，1包含
                    {
                        this.UseLimitFreeNoServer(buyRatio);
                    } else {
                ...
                case MTTJoinMode.AddOn:
                    if (
                        this._data.addon_free_incl_svr == 0
                    ) //限免是否包含服务费，0不包含，1包含
                    {
                        this.UseLimitFreeNoServer(buyRatio);
                    } else {
        ...
        if (this.isUseMultFree) {
            if (
                this._data.multi_ratio_free_incl_svr == 0
            ) //限免是否包含服务费，0不包含，1包含
            {
                this.UseLimitFreeNoServer(buyRatio);
            } else {
    ...
    signUpReq() {
        if (ClubCache.mttPayWallat.gold < this._coinnum) {
            UIComponent.Instance.OpenNoAnimation(
                UIDefine.UINewDialogComponent,
                {
            ...
            UIMTTModel.Instance.APIPropUserBuyProp((response) => {
                if (response.code == 0) {
                    if (null != this._data && null != this._data.actionCommit) {
                        this._data.actionCommit.Invoke(
                            true,
                            1,
                            this.used_prop_id,
                            this.prop_type,
                    ...
                    ToastManager.Instance.createToast(
                        CPErrorCode.ServerErrorDescription(response.code),
                    );
                }
            });
        }
