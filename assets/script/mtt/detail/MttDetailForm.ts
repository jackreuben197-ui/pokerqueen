...
import {
    ...
    WebRoomCenterMttHranks,
    WebRoomCenterMttMyawArd,
    WebRoomCenterMttRanks,
    WebRoomCenterMttRealPrize,
    WebRoomCenterMttRooms
...
export default class MttDetailForm extends BaseForm {
    ...
    refreshStatusUI(res) {
            ...
            if (i == 1) {
                ...
                if (mttDetails.mtt.prop_buy_type == 0) {
                    ...
                    if (mttDetails.mtt.apply_fee_pool + mttDetails.mtt.apply_fee_service + mttDetails.mtt.apply_fee_hunter <= 0) {
                        ...
                        let num = mttDetails.mtt.apply_fee_pool + mttDetails.mtt.apply_fee_service + mttDetails.mtt.apply_fee_hunter;
                        lbl_gold.string = `${(i18nMgr.Get('UIMatch_MttDetailState_ReyBuFerr02'), num * 0.01)}`;
                    }
                }
            }
            if (!isStar) {
                ...
                lbl_time.string =
                    ...
                    ymd.year +
                    ' - ' +
                    TimeHelper.toTimeFormat(date.getHours()).toString() +
                    ':' +
                    TimeHelper.toTimeFormat(date.getMinutes()).toString();
            }
            this.isStar = isStar;
        }
        ...
        for (let i = 1; i < 10; i++) {
            ...
            if (i == 1) {
                ...
                lbl_2.string =
                    `${mttDetails.mtt.initial_score * 0.01}` +
                    '(' +
                    `${mttDetails.mtt.initial_score / (MTTGameUtil.BlindAtLevel(0, mttDetails.mtt.blindtable_type, 1) * 2)}` +
                    'BB)';
            } else if (i == 3) {
                ...
                if (mttDetails.mtt.apply_fee_hunter > 0) {
                    ...
                    lbl_3.string = i18nMgr
                        .Get('UIMTT_StateHuntChampionshipsDetail')
                        .replace('{0}', ' ' + (mttDetails.mtt.apply_fee_hunter / 100).toString() + ' ');
                } else {
                ...
                if (mttDetails.mtt.max_delay_apply_bl > mttDetails.more.bl) {
                    if (mttDetails.mtt.addon_begin_bl == 0 && mttDetails.mtt.addon_end_bl == 0) {
                        lbl_2.string = LobbyControl.getInstance().formatString(
                            i18nMgr.Get('MTT_State_DelayDetailNoAddOn'),
                            mttDetails.mtt.limit_total_buy_times.toString(),
                            mttDetails.mtt.max_delay_apply_bl
                        );
                    } else {
                        lbl_2.string = LobbyControl.getInstance().formatString(
                            i18nMgr.Get('MTT_State_DelayDetail'),
                            mttDetails.mtt.limit_total_buy_times.toString(),
                            mttDetails.mtt.max_delay_apply_bl,
                            mttDetails.mtt.addon_begin_bl,
                            mttDetails.mtt.addon_end_bl
                        );
                    }
                } else {
                ...
                lbl_2.string =
                    StringHelper.GetLongString(mttDetails.more.sb) +
                    '/' +
                    StringHelper.GetLongString(mttDetails.more.sb * 2) +
                    '(' +
                    StringHelper.GetLongString(mttDetails.more.ante) +
                    ')';
            } else if (i == 7) {
                //下一盲注
                if (mttDetails.more.nbl) {
                    ...
                    lbl_2.string =
                        StringHelper.GetLongString(mttDetails.more.nsb) +
                        '/' +
                        StringHelper.GetLongString(mttDetails.more.nsb * 2) +
                        '(' +
                        StringHelper.GetLongString(mttDetails.more.nante) +
                        ')';
                } else {
                    baseNode.active = false;
                }
    ...
    refreshListView(index, data) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (index == 5) {
                ...
                lbl_time.getComponent(cc.Label).string = i18nMgr
                    .Get('UITexasReport_Text_MatchNextBlindTime')
                    .replace('{0}', (data.mtt.upblind_interval / 60).toString());
    ...
    RefreshMttDetails(callback = null) {
        ...
        UIMTTModel.Instance.RequestMTTDetails(
            ...
            code => {
                if (code == 0) {
                    this.UpdateBtn();
                    this.refreshStatusUI(UIMTTModel.Instance.MttInfo);
                    //声纹获取麦克风权限
                    if (UIMTTModel.Instance.MttInfo.mtt.voiceprint_verify_on == 1) {
                    ...
                    if (UIMTTModel.Instance.MttInfo.mtt.buy_prop_id != 0) {
                        UIMTTModel.Instance.APIPropUserCheckPropInfo(res => {
                            if (res.code == 0) {
                                ...
                                //     mUIComponent.UpdateInfo(UIMTTModel.Instance.MttInfo);
                                // }
                            } else {
                    ...
                    //判断当前时间是否大于进入比赛时间
                    this.isCurTimeOverEnterTime = TimeHelper.Now >= TimeHelper.RFC3339TimeConvertToUTCTime(UIMTTModel.Instance.MttInfo.mtt.enter_time);
                    if (callback) {
                        callback();
                    }
                } else {
    ...
    update() {
        if (UIMTTModel.Instance.MttInfo == null || this.isCurTimeOverEnterTime) {
            return;
        }
        if (TimeHelper.Now >= TimeHelper.RFC3339TimeConvertToUTCTime(UIMTTModel.Instance.MttInfo.mtt.enter_time)) {
            this.RefreshMttDetails();
                        ...
                        UIMTTModel.Instance.HandleMTTJoinAction(
                            ...
                            bringInCode => {
                                if (bringInCode == 0) {
                                    ...
                                    // UIMTTModel.Instance.ShowGameplayUI(fromUI: UIType.UIMatch_MttDetail, isLookOn: false, roomid: 0);
                        ...
                        UIMineModel.mInstance.ObtainUserInfo(pDto => {
                            UIMTTModel.Instance.HandleMTTJoinAction(
                                ...
                                rebuyCode => {
                                    if (rebuyCode == 0) {
                                        UIMTTModel.Instance.ShowGameplayUI(false, 0);
                                        //UIMTTModel.Instance.ShowGameplayUI([UIDefine.MttDetailForm, UIDefine.MttListForm], false, 0);
                                        // UIMTTModel.Instance.ShowGameplayUI(fromUI: UIType.UIMatch_MttDetail, isLookOn: false, roomid: 0);
                                    } else {
                                        this.RefreshMttDetails();
    ...
    UpdateBtn() {
        ...
        switch (UIMTTModel.Instance.MttInfo.state_code) {
            ...
            case MTTGame.MTTPlayerStatus.Lose:
                {
                    if (UIMTTModel.Instance.MttInfo.more.bl >= UIMTTModel.Instance.MttInfo.mtt.max_rebuy_bl) {
                        lbl_signUp.string = i18nMgr.Get('mtt_btn_Stopbuying');
                    } else {
                        lbl_signUp.string =
                            i18nMgr.Get('MTT_Rebuy') +
                            ' ' +
                            UIMTTModel.Instance.MttInfo.state.left_rebuy_times +
                            '/' +
                            UIMTTModel.Instance.MttInfo.mtt.rebuy_times;
                    }
                }
                break;
