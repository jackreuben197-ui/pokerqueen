...
import {
    APIOrgTribeRoomPermissions,
    APIOrgUserNewLabelRead,
    APIOrgUserNewLabelReadNum,
    WebConfigGlobalConfig as Web_Config_Global_Config,
    WWW
...
export default class UIGameplayTableSetting extends BaseTouchBoard {
    ...
    private RefreshUI(): void {
        ...
        const sixPlusAndBombpot = (GameCache.Instance.game_type === GameType.Holdem && GameCache.Instance.poker_type === 2) || !!game?.isBombPot;
        ...
        if (this.IsPermitted('room_encrypt_cards')) {
            ...
            this.FillData(
                'room_encrypt_cards',
                this.L('UIBlockchain'),
                this.L(blockchainType === 1 ? '6digit_password_opened' : 'UIMine_AccountNotOpen'),
                blockchainType === 1 ? '#7ED27E' : '#FF6666'
            );
        }
    ...
    private GetTableTypeName(): string {
        ...
        if (GameCache.Instance.game_type >= GameType.Omaha4 && GameCache.Instance.game_type <= GameType.Omaha6 && GameCache.Instance.poker_type === 0) {
