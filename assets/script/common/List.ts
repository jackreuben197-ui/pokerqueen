    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            case SelectedType.SINGLE: {
                ...
                if (t._selectedId >= 0)
                    t._lastSelectedId = t._selectedId; //如果＜0则取消选择，把_lastSelectedId也置空吧，如果以后有特殊需求再改吧。
    ...
    update() {
        ...
        if (this._virtual) {
            ...
            if (this._updateCounter < this._numItems) {
                let len: number =
                    this._updateCounter + this.frameByFrameRenderNum > this._numItems ? this._numItems : this._updateCounter + this.frameByFrameRenderNum;
                for (let n: number = this._updateCounter; n < len; n++) {
