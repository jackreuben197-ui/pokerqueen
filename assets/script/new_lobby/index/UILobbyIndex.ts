...
export default class UILobbyIndex extends UIBasePlus {
    ...
    */
    ...
    protected onScrolling(offset: cc.Vec2, scrollSpeed: number = 0): void {
        ...
        let contentWorldPos = this.mScrollView.content.convertToWorldSpaceAR(
            cc.Vec2.ZERO,
        );
    ...
    protected lateLoad(): void {
        ...
        if (this.cc_Label$noticelabel) {
            this.cc_Label$noticelabel.string =
                "hello,world! 此处播放广播消息，长度过长的话，会被截断。";
        }
        ...
        if (this.cc_Button$game1) {
            this.cc_Button$game1.node.on(
                cc.Node.EventType.TOUCH_END,
                this.onGame1Click,
                this,
            );
            this.cc_Button$game2.node.on(
                cc.Node.EventType.TOUCH_END,
                this.onGame2Click,
                this,
            );
            this.cc_Button$service.node.on(
                cc.Node.EventType.TOUCH_END,
                this.onServiceClick,
                this,
            );
    ...
    protected onPoker(event: cc.Event.EventTouch): void {
        console.log("扑克区域被点击.");
        UIComponent.open(UIDefine.UIPokerRoomList, null, {
    ...
    refreshRooms() {
        ...
        if (this.curr_room_list.length) {
            this.curr_room_list.forEach((room, index) => {
                ...
                let item_sc: ItemLobbyRoom =
                    item_node.getComponent(ItemLobbyRoom);
                item_sc.onShow(room);
                item_node["room"] = room;
