...
export default class UIFriendMatch extends UIBase {
    ...
    protected lateLoad(): void {
        ...
        GC.notify.register(
            ...
            this.ProtocolHoldemGetMsgHandler,
            this,
        ); // 广播表情
    }
    ...
    async onShow(param?: any, fromUI?: cc.Node, sceneUI?: cc.Node) {
        ...
        for (let index = 0; index < this.numNode.childrenCount; index++) {
            const element = this.numNode.children[index]
    ...
    onDiamondClick() {
        //跳转商城
        UIComponent.open(UIDefine.UIMall, null, {
            SceneUI: SceneManager.Instance.currUI,
        });
    }
    ...
    titleNodeClick(event, customData) {
        ...
        this.pjlbl.node.color =
            this._selectTitle == 1
                ? cc.color().fromHEX("#FFFFFF")
                : cc.color().fromHEX("#E6E8EC");
        this.datalbl.node.color =
            this._selectTitle == 2
                ? cc.color().fromHEX("#FFFFFF")
                : cc.color().fromHEX("#E6E8EC");
    }
    ...
    async reqDataAgain() {
        let result: any = await UIClubModel.mInstance
            .WebOrgFriendRoomList(false)
            .catch((content) => {
                console.log(
                    `>> catch error:${WebOrgFriendRoomList.API}`,
                    content,
                );
            });
        if (!result) return;
    ...
    createMatch() {
        UIComponent.open(UIDefine.UIClubCreateMatchHome, 1, {
            SceneUI: SceneManager.Instance.currUI,
        });
    }
    numNodeClick() {
        UIComponent.open(UIDefine.UIKeyNode, {
            cb: this.ketNodeCall.bind(this),
        });
    }
    ketNodeCall(data) {
        switch (Number(data)) {
