...
import { WWW, WebGuildGiveRecyCle } from "../../../net/https/WebRequest";
...
export default class UIFunRecycleGive extends BaseFormPlus {
    ...
    data: { balance: number; op_type: number; gold_type: number; list: any } =
        null; // type 1 发放， 2 回收
    ...
    refreshUI(data: any) {
        ...
        this.setChildVisible(
            this.$keyboard,
            "show/icon/uc",
            this.data.gold_type == 1,
        );
        this.setChildVisible(
            this.$keyboard,
            "show/icon/gc",
            this.data.gold_type == 2,
        );
        ...
        if (data.op_type == 1) {
            ...
            this.title_label.i18NString = i18nMgr.Get(
                "UIClub_FundDetail_5iSXE2Uj",
            );
            ...
            this.setChildLabel(
                this.$btn_commit,
                "label",
                i18nMgr.Get("UIClub_FundDetail_5iSXE2Uj"),
            );
        } else {
            //回收
            ...
            this.setChildLabel(
                this.$btn_commit,
                "label",
                i18nMgr.Get("UIClub_FundDetail_recycle"),
            );
        }
    }
    ...
    getMaxRecycle() {
        ...
        this.tempIndexs.forEach((index) => {
            let gold =
                this.data.gold_type == 1
                    ? this.data.list[index].gold
                    : this.data.list[index].usdt;
    ...
    onClickCommit() {
        if (this.data.op_type == 1) {
            //发放
            if (
                +this.cc_Label$show_label.string * +this.cc_Label$count.string >
                this.data.balance
            ) {
            ...
            if (
                +StringHelper.GetLongString(this.getMaxRecycle()) <
                +this.cc_Label$show_label.string
            ) {
        ...
        WWW.Instance.CommonAPI({
            ...
            (res: any) => {
                ...
                UIComponent.Instance.getComponent<UIFunManage>(
                    "UIFunManage",
                ).refreshPanel();
            },
            (res: any) => {
    ...
    render_item(node: cc.Node, index: number) {
        ...
        node.getComponent(FunMemberItem).onShow({
            data: item_data,
            index: index,
            gold_type: this.data.gold_type,
            check: check,
    ...
    key_click(button: cc.Button) {
        ...
        if (value == "X") {
            ...
            if (last == 0) {
                ...
                this.cc_Label$show_label.string =
                    this.cc_Label$show_label.string.substring(0, last);
            }
        ...
        if (
            dot_index > -1 &&
            dot_index == this.cc_Label$show_label.string.length - 3
        )
            return;
