...
export default class ThrowPropManager {
    ...
    private static readonly CONFIGS: PropAnimConfig[] = [
        ...
        // 10: 鱼头 (610) — 模式D
        { pattern: 'D', spines: ['spine/Expression_Fish/sy3', 'spine/Expression_Fish_Screen_Sender/sy2', 'spine/Expression_Fish_Screen_Receiver/sy', 'spine/Expression_Fish_Wave/hl'], anims: [] },
        // 11: 棒球 (611) — 模式D
        { pattern: 'D', spines: ['spine/Expression_BaseBall_Sender/skeleton', 'spine/Expression_BaseBall_Receiver/ballfolder1', 'spine/Expression_BaseBall_Other/skeleton'], anims: [] },
    ];
    ...
    handlePropMessage(broadcastMsg: { type: number; user_id: number; target_user_id: number }): void {
        ...
        switch (config.pattern) {
            case 'A': this.playPatternA(offset, config, senderSeat, targetSeat); break;
            case 'B': this.playPatternB(offset, config, senderSeat, targetSeat); break;
            case 'C': this.playPatternC(offset, config, senderSeat, targetSeat); break;
            case 'D': this.playPatternD(offset, senderSeat, targetSeat); break;
        }
    }
    ...
    private playPatternA(offset: number, config: PropAnimConfig, senderSeat: Seat, targetSeat: Seat): void {
        ...
        this.loadSkeleton(config.spines[0]).then(skeletonData => {
    ...
    private playPatternC(offset: number, config: PropAnimConfig, senderSeat: Seat, targetSeat: Seat): void {
        ...
        this.loadSkeletons(config.spines).then(allData => {
            if (!cc.isValid(root)) return;
            ...
            if (config.anims.length === 1) {
                ...
                if (offset === 9) {
                    const role = this.getRole(
                        senderSeat.Player ? senderSeat.Player.userID : 0,
                        targetSeat.Player ? targetSeat.Player.userID : 0
                    );
                    if (role === 'receiver') {
    ...
    private playPatternD(propOffset: number, senderSeat: Seat, targetSeat: Seat): void {
        ...
        switch (propOffset) {
            case 4: this.playBeer(root, senderSeat, targetSeat, role); break;
            case 8: this.playBoxing(root, senderSeat, targetSeat, role); break;
            case 10: this.playFish(root, senderSeat, targetSeat, role); break;
            case 11: this.playBaseball(root, senderSeat, targetSeat, role); break;
        }
    }
    ...
    private playBeer(root: cc.Node, senderSeat: Seat, targetSeat: Seat, role: string): void {
        ...
        this.loadSkeletons(['spine/Expression_Beer/cheers_2', 'spine/Expression_Beer_Screen/cheers_1']).then(([beerData, screenData]) => {
            if (!cc.isValid(root)) return;
    ...
    private playBoxing(root: cc.Node, senderSeat: Seat, targetSeat: Seat, role: string): void {
        ...
        this.loadSkeletons(['spine/Expression_Box/box_local', 'spine/Expression_Box_Screen/box_full']).then(([boxData, screenData]) => {
            if (!cc.isValid(root)) return;
    ...
    private playFish(root: cc.Node, senderSeat: Seat, targetSeat: Seat, role: string): void {
        ...
        this.loadSkeletons(paths).then(([fishData, senderScreenData, receiverScreenData, waveData]) => {
            if (!cc.isValid(root)) return;
            ...
            if (role === 'sender') {
                ...
                cc.tween(fishNode)
                    ...
                    .delay(1.0)
                    .call(() => { if (cc.isValid(fishNode)) fishNode.destroy(); })
                    .start();
    ...
    private playBaseball(root: cc.Node, senderSeat: Seat, targetSeat: Seat, role: string): void {
        ...
        this.loadSkeletons(paths).then(([senderData, receiverData, otherData]) => {
            if (!cc.isValid(root)) return;
    ...
    private playBaseballSequence(
        skeletonData: sp.SkeletonData, root: cc.Node,
        startPos: cc.Vec3, targetPos: cc.Vec3,
        animNames: string[], moveDuration: number
    ): void {
        const node = new cc.Node('PropSpine');
        const skeleton = node.addComponent(sp.Skeleton);
        skeleton.skeletonData = skeletonData;
        ...
        skeleton.setCompleteListener(() => {
            ...
            cc.tween(node)
                .to(moveDuration, { x: targetPos.x, y: targetPos.y }, { easing: 'quadInOut' })
                .start();
            // 阶段2完成后播放阶段3
    ...
    private playBaseballBystander(
        skeletonData: sp.SkeletonData, root: cc.Node,
        senderPos: cc.Vec3, targetPos: cc.Vec3
    ): void {
        const node = new cc.Node('PropSpine');
        const skeleton = node.addComponent(sp.Skeleton);
        skeleton.skeletonData = skeletonData;
        ...
        skeleton.setCompleteListener(() => {
            ...
            cc.tween(node)
                .to(0.333, { x: targetPos.x, y: targetPos.y }, { easing: 'quadInOut' })
                .start();
            cc.tween(node)
                .delay(0.333)
                .call(() => {
                    ...
                    skeleton.setCompleteListener(() => {
                        ...
                        const exitPos = cc.v3(
                            targetPos.x + moveDir.x * 2000,
                            targetPos.y + moveDir.y * 2000,
                            0
                        );
                        cc.tween(node)
    ...
    playDiamondAnimation(senderId: number, receiverId: number, amount: number): void {
        ...
        cc.resources.load('effect/diamondFly', cc.Prefab, (err, flyPrefab: cc.Prefab) => {
            if (err) { console.warn(LN, 'diamondFly load failed:', err.message); return; }

            cc.resources.load('effect/DiamondIcon', cc.Prefab, (err2, iconPrefab: cc.Prefab) => {
                if (err2) { console.warn(LN, 'DiamondIcon load failed:', err2.message); return; }

                cc.resources.load('effect/diamondSpine', cc.Prefab, (err3, spinePrefab: cc.Prefab) => {
                    if (err3) { console.warn(LN, 'diamondSpine load failed:', err3.message); return; }
                    if (!cc.isValid(root)) return;
    ...
    private playDiamondSpineEffect(prefab: cc.Prefab, parentNode: cc.Node, localPos: cc.Vec3): void {
        ...
        if (skeleton) {
            ...
            cc.tween(node).delay(2).call(() => { if (cc.isValid(node)) node.destroy(); }).start();
        }
    }
