...
import { traceClass } from "../../../core/LogTrace";
...
export default class SeatPlayer extends cc.Component {
    ...
    private onUpdatePosition(pos: SeatPosition, pat: AnimateDisplayTypePosition) {
        this.tracelog.debug('update positon', this._seatPlayer.mine, this._seatPlayer.seatNo)
        switch(pos){
    ...
    private onWin() {
        ...
        cc.tween(this.animatingChips)
            ...
            .call(() => { this.animatingChips.active = false; })
            .start();
    ...
    public animateButtonChange(enable: boolean, positionFromNode?: cc.Node) {
        ...
        if (enable && positionFromNode) {
            ...
            cc.tween(this.buttonIcon)
                .to(0.6, { x: endPos.x, y: endPos.y }, { easing: 'cubicOut' })
                .start();
        }
    }
}