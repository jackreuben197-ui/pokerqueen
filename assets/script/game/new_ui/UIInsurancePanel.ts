...
export default class UIInsurancePanel extends UIBasePlus {
    ...
    private UpdatePublicCards() {
        if (null == this.data.publicCards) return;
        for (let i = 0; i < 5; i++) {
        ...
        for (let i = 0; i < 9; i++) {
            if (i < len) {
                ...
                if (i == 0) {
                    ...
                    this.player_items[i].UpdateItem(
                        this.myWrapTriggedInsuranceData.playerCards[k],
                        this.myWrapTriggedInsuranceData.userNames[k],
                        this.myWrapTriggedInsuranceData.outsPerUser[k]
                    );
                }
            } else {
    ...
    private UnSelectedOdd() {
        if (this.userOutsCardsData.equalOuts != null) {
            //如果有平分outs，再减去平分
            return GameUtil.GetOddsByPlayerNum(
                this.myWrapTriggedInsuranceData.PotUserCount,
                this.userOutsCardsData.overOuts.length + this.userOutsCardsData.equalOuts.length - this.selectOuts
            );
        } else {
        ...
        if (this.myWrapTriggedInsuranceData.potAllowOutSelection == 1) {
            ...
            if (this.userOutsCardsData.equalOuts != null) {
                if (this.selectOuts != this.userOutsCardsData.overOuts.length + this.userOutsCardsData.equalOuts.length) {
                    ...
                    this.cc_Label$tips.string = CPErrorCode.LanguageDescription(20034, [
                        this.userOutsCardsData.overOuts.length + this.userOutsCardsData.equalOuts.length - this.selectOuts,
                        this.UnSelectedOdd(),
                        autoInsured / 100
                    ]);
                ...
                if (this.selectOuts != this.userOutsCardsData.overOuts.length) {
                    let autoInsured = Math.ceil(this.SliderPlus$slider.value / this.UnSelectedOdd());
                    this.cc_Label$tips.string = CPErrorCode.LanguageDescription(20034, [
                        this.userOutsCardsData.overOuts.length - this.selectOuts,
                        this.UnSelectedOdd(),
                        autoInsured / 100
                    ]);
                } else {
        ...
        if (this.myWrapTriggedInsuranceData.potAllowOutSelection == 1) {
            ...
            if (this.allSelect) {
                for (let i = 0; i < this.listInsuranceCardItems.length; i++) {
                    ...
                    if (mInsuranceCardItem.IsSelect) this.selectOuts++;
                    else this.selectOuts--;
                }
                this.UpdateOuts();
                this.UpdateInsuranceSlider();
                this.SliderPlus$slider.value = this.CurrentSecureAmount();
