...
export default class UITexasJackpotRecentAwardRecord extends BaseTouchBoard {
    ...
    protected lateLoad(): void {
        ...
        this.totalContent =
            (this.totalScrollView && cc.find('view/content', this.totalScrollView)) || this.totalScrollView?.getChildByName('content') || this.totalScrollView;
    ...
    private FillJackpot(setting: any): void {
        ...
        if (contributeType === 2) {
            this.poolRewardContent.string = StringHelper.Format(i18nMgr.Get('UIJackpotRecentAwardRecord_PoolReward2'), [
                Number(blind?.contribute_ratio || 0) / 10
            ]);
        } else if (contributeType === 3) {
            this.poolRewardContent.string = StringHelper.Format(i18nMgr.Get('UIJackpotRecentAwardRecord_PoolReward3'), [
                Number(blind?.contribute_pot_ratio || 0) / 10
            ]);
        } else {
            this.poolRewardContent.string = StringHelper.Format(i18nMgr.Get('UIJackpotRecentAwardRecord_PoolReward1'), [
                Number(blind?.contribute_fixed_limit || 0),
                Number(blind?.contribute_fixed_rate || 0)
            ]);
        }
    }
    ...
    private FillAwardData(items: JackpotAwardItem[]): void {
        ...
        if (this.listTip) {
            this.listTip.string = items.length <= 1 ? i18nMgr.Get('UIClub_FundDetail_xYlV8VBZ') : i18nMgr.Get('UIJackpotRecentAwardRecord_PoolRecond');
        }
