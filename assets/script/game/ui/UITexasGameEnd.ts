...
export default class UITexasGameEnd extends UIBase {
    ...
    private async GetGameEndData(): Promise<void> {
        ...
        const response: typeof Web_User_Room_Settle_Detail.Response = await UITexasModel.mInstance.APIUserRoomSettleDetail(this.mRoomId);
        if (!response || !this.node.activeInHierarchy) {
            return;
        }
