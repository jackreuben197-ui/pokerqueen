...
export default class UIMTTMineRankComponent extends UIBase {
    ...
    btn_ok: cc.Node = null;
    panel_show: cc.Node = null;

    panel_gift: cc.Node = null;
    node_list2: cc.Node = null;
    item_Award: cc.Node = null;
    ...
    InItUI()
    {
        ...
        this.panel_gift = this.getChildNodeOrComponent("panel_gift");
        this.node_list2 = this.getChildNodeOrComponent("node_list2");
    ...
    onShow(obj?: any): void {
        ...
        this.btn_ok = btn_ok;
        this.panel_show = this.getChildNodeOrComponent("panel_show");
...
// #endif
    ...
    {
        this.panel_gift.removeAllChildren();
        ...
        if (responseData.data.award_gold > 0)
        {
            ...
            // go.SetActive(true);
            let go = cc.instantiate(this.item_Award);
            go.x = 0;
            go.y = 0;
            this.panel_gift.addChild(go);
            this.SetItemData(go, responseData.data.award_gold, false, null, responseData);
            //BgHeight += 260;
            awardTypeNum++;
        }
        if (responseData.data.hunter_award > 0)
        {
            ...
            go.y = 0;
            this.panel_gift.addChild(go);
            this.SetItemData(go, responseData.data.hunter_award, true, null, responseData);
            // BgHeight += 260;
            awardTypeNum++;
        }
        if (responseData.data.award_goods != null)
        {
            for (let i=0; i<responseData.data.award_goods.length; i++)
            {
                if (awardTypeNum >= 2)
                {
                    break;
                ...
                go.y = 0;
                this.panel_gift.addChild(go);
                this.SetItemData(go, responseData.data.hunter_award, false, responseData.data.award_goods[i], responseData);
                awardTypeNum++;
            }
        }
    }
    ...
    SetItemData(gameObject, gold, isHunter, awardGoods, responseData)
    {
        ...
        let propu_icon = gameObject.getChildByName("propu_icon");
        let Image = gameObject.getChildByName("Image");
        let Text_reward = gameObject.getChildByName("Text_reward");
        let Text_several = gameObject.getChildByName("Text_several");
        let icon_name = Image.getChildByName("icon_name");
        if (awardGoods != null)
        {
            //MemberExtraAward.gameObject.SetActive(false);
            // Log.Debug($"{GlobalData.Instance.PropIconUrl}image-normal/prop-{awardGoods.i}.png");
            let GlobalProto = GameConfig.GlobalProto;
            let iconStr = `${GlobalProto.PropIconUrl}image-normal/prop-${awardGoods.i}.png`
            WebImageHelper.SetUrlImage(propu_icon.getComponent(cc.Sprite), iconStr);
          
            Image.active = false;
            propu_icon.active = true;

            Text_reward.getComponent(cc.Label).string = GC.data.languageTemp.temp.getName(awardGoods.na);
            Text_several.getComponent(cc.Label).string = "X" + awardGoods.n;
        }
        else
        {
            if (isHunter)
            {
                Text_reward.getComponent(cc.Label).string = i18nMgr.Get("MTT_end_Hunter_reward"), StringHelper.GetSignedLongString(gold);

                Text_several.getComponent(cc.Label).string = "";
                icon_name.getComponent(cc.Label).string = i18nMgr.Get("UIMine_WalletSC001");
                Image.active = true;
                propu_icon.active = false;
            }
            else
            {
                Text_several.active = false;
                ...
                Text_reward.getComponent(cc.Label).string = i18nMgr.Get("MTT_end_normal_reward").replace("{0}", StringHelper.GetSignedLongString(gold))
            }
        }
    }
