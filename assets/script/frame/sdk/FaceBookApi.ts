...
export default class FaceBookApi {
    ...
    static login(func: Function) {
        this.func = func;
        (window as any).FB &&
            (window as any).FB.getLoginStatus(function (response: TFackBookLoginCB) {
                // 返回的数据结构
                // {
                //     status: 'connected',
                //     authResponse: {
    ...
    static loginSuc(msg: TFackBookLoginRspAuth) {
        ...
        if (this.func && this.func instanceof Function) {
            ...
            LoginSession.WebLoginThirdParty({ token: this.rspData.accessToken, source: 'facebook', app_source: 3 }).then(() =>
