...
export default class MttListItemModel {
    ...
    constructor(msg: TMttListItem) {
        ...
        this._upblindInterval =
            new Date(this._msg.upblind_interval).getTime() / 1000;
        this._maxDelayApplyBl =
            new Date(this._msg.max_delay_apply_bl).getTime() / 1000;
    }
    ...
    get buyIn() {
        return (
            this.apply_fee_pool + this.apply_fee_service + this.apply_fee_hunter
        );
    }
    ...
    get delayApplyEndTime() {
        return (
            this.start_time + this.upblind_interval * this.max_delay_apply_bl
        );
    }
}
