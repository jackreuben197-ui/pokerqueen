...
export default class UIMTTDetail_Blind extends UIBasePlus {
    ...
    refreshTop(res?: any) {
        if (res) {
            ...
            let levelCount = MTTGameUtil.numOfLevel(
                this.mtt_detail.mtt.blindtable_type,
            );

            this.$top_blind.children[0].getComponent(cc.Label).string =
                StringHelper.Format(i18nMgr.Get("MTT_Blind_Num"), [levelCount]);
    ...
    render_item(node: cc.Node, index: number) {
        ...
        this.setChildVisible(
            node,
            "arrow",
            this.mtt_detail.mtt.max_rebuy_bl == level,
        );
        ...
        this.setChildLabel(
            node,
            "bl",
            i18nMgr.Get("MTT_Blind_Deadline_to_buy"),
        );
        this.setChildColor(node, "bl", TextColor.Color6);

        if (
            this.mtt_detail.mtt.addon_begin_bl == level &&
            this.mtt_detail.mtt.addon_begin_bl > 0
        ) {
            this.setChildLabel(
                node,
                "bl",
                i18nMgr.Get("MTT_Blind_Deadline_add_op"),
            );
            this.setChildColor(node, "bl", TextColor.Color5);
        }
        if (
            this.mtt_detail.mtt.addon_end_bl == level &&
            this.mtt_detail.mtt.addon_end_bl > 0
        ) {
            this.setChildLabel(
                node,
                "bl",
                i18nMgr.Get("MTT_Blind_Deadline_add_cl"),
            );
            this.setChildColor(node, "bl", TextColor.Color8);
        }
        let sb = MTTGameUtil.BlindAtLevel(
            index,
            this.mtt_detail.mtt.blindtable_type,
            1,
        ); //缺少字段，倍数暂时*1，有需求，需要加字段
        let ante = MTTGameUtil.AnteAtLevel(
            index,
            this.mtt_detail.mtt.blindtable_type,
            1,
        );
        ...
        this.setChildLabel(
            node,
            "blind",
            `${StringHelper.GetLongStringUnit(sb) + "/" + StringHelper.GetLongStringUnit(sb * 2)}`,
        );
        this.setChildLabel(
            node,
            "ante",
            `${StringHelper.GetLongStringUnit(ante)}`,
        );
        this.setChildLabel(
            node,
            "time",
            `${StringHelper.Format(i18nMgr.Get("UITexasReport_Text_MatchNextBlindTime"), [this.mtt_detail.mtt.upblind_interval / 60])}`,
        );
    }
}
