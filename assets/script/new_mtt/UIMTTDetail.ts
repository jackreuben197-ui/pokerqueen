...
export default class UIMTTDetail extends BaseFormPlus {
    ...
    protected lateLoad() {
        ...
        super.lateLoad();
        this.indexs_group = new TabsGroup(
            this.$indexs.children,
            this.click_index,
            this,
        );
        this.pages = [
        ...
        if (data.mtt.buy_prop_id != 0) {
            WWW.Instance.CommonAPI({
                ...
                (res: any) => {
                    ...
                    this.$mtt_state
                        .getComponent(UIMTTDetail_State)
                        .UpdateInfo(UIMTTModel.Instance.MttInfo, this._param);
                },
                (res: any) => {},
            );
        } else {
    ...
    refreshDetail(data: any) {
        this.cc_Label$mtt_name.string = LobbySession.getLanguageValueByKey(
            data.mtt.name,
        );
        ...
        //判断当前时间是否大于进入比赛时间
        this.is_over_time =
            TimeHelper.Now > new Date(data.mtt.enter_time).getTime();
    }
        ...
        switch (data.state_code) {
            case MTTGame.MTTPlayerStatus.WaitingApply:
                this.setChildLabel(
                    this.$btn_signup,
                    "label",
                    i18nMgr.Get("mtt_btn_waiting_start"),
                );
                break;
            ...
            case MTTGame.MTTPlayerStatus.AppliedNotStart:
                ...
                this.setChildLabel(
                    this.$btn_signup,
                    "label",
                    i18nMgr.Get("Mtt_AppliedNotStart"),
                );
            ...
            case MTTGame.MTTPlayerStatus.CannotApplyStarted:
                this.setChildLabel(
                    this.$btn_signup,
                    "label",
                    i18nMgr.Get("mtt_btn_sign_up_deadline"),
                );
            ...
            case MTTGame.MTTPlayerStatus.Lose:
                if (data.more.bl >= data.mtt.max_rebuy_bl) {
                    this.setChildLabel(
                        this.$btn_signup,
                        "label",
                        i18nMgr.Get("mtt_btn_Stopbuying"),
                    );
                } else {
                    this.setChildLabel(
                        this.$btn_signup,
                        "label",
                        `${i18nMgr.Get("MTT_Rebuy")} ${data.state.left_rebuy_times}/${data.mtt.rebuy_times}`,
                    );
                }
            ...
            case MTTGame.MTTPlayerStatus.CannotJoinOvertime:
                ...
                this.setChildLabel(
                    this.$btn_signup,
                    "label",
                    i18nMgr.Get("Mtt_CannotJoinOvertime"),
                );
    ...
    protected update(dt: number): void {
        ...
        if (
            TimeHelper.Now >=
            new Date(UIMTTModel.Instance.MttInfo.mtt.enter_time).getTime()
        ) {
            this.reqMTTDetail();
    ...
    click_index(items: cc.Node[], index: number) {
        ...
        items.forEach((item, index) => {
            ...
            this.setChildColor(
                item,
                "label",
                status ? TextColor.Color7 : TextColor.Color3,
            );
            ...
            if (this.pages[index]?.active) {
                this.pages[index]
                    .getComponent(UIBasePlus)
                    .onShow(UIMTTModel.Instance.MttInfo);
            }
        });
        //////////////////////////////////
        switch (index) {
                ...
                case MTTGame.MTTPlayerStatus.LoseCanRebuy:
                    UIMineModel.mInstance.ObtainUserInfo((pDto) => {
                        UIMTTModel.Instance.HandleMTTJoinAction(
                            ...
                            (rebuyCode) => {
                                if (rebuyCode == 0) {
                                    UIMTTModel.Instance.ShowGameplayUI(
                                        false,
                                        0,
                                    );
                                } else {
