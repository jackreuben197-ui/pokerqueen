...
export default class UIMTTDetail_State extends UIBasePlus {
    ...
    UpdateInfo(mtt_detail: any, list_item_data: any) {
        ...
        this.$GameType.children[1].getComponent(cc.Label).string =
            mtt_detail.mtt.poker_type == 2 ? this.six_List[mtt_detail.mtt.game_type] : this.type_List[mtt_detail.mtt.game_type];
        //起始记分牌
        this.$Qishi.children[0].getComponent(cc.Label).string = i18nMgr.Get('MTT_State_Starting_Scoreboard');
        this.$Qishi.children[1].getComponent(cc.Label).string =
            `${StringHelper.GetDecimalN(mtt_detail.mtt.initial_score / 100)}(${StringHelper.GetDecimalN(mtt_detail.mtt.initial_score / (MTTGameUtil.BlindAtLevel(0, mtt_detail.mtt.blindtable_type, 1) * 2))}BB)`; // /roomInfo.mtt.sb[0] * 2 / 100
        if (mtt_detail.mtt.hunter_on > 0) {
            ...
            this.$HuntReward.children[2].getComponent(cc.Label).string = StringHelper.Format(i18nMgr.Get('UIMTT_StateHuntChampionshipsDetail'), [
                `${StringHelper.GetDecimalN(mtt_detail.mtt.apply_fee_hunter / 100)}`
            ]);
        ...
        if (mtt_detail.mtt.max_delay_apply_bl > mtt_detail.more.bl) {
            if (mtt_detail.mtt.addon_begin_bl == 0 && mtt_detail.mtt.addon_end_bl == 0) {
                this.$Shangxian.children[1].getComponent(cc.Label).string = StringHelper.Format(i18nMgr.Get('MTT_State_DelayDetailNoAddOn'), [
                    `${mtt_detail.mtt.limit_total_buy_times}`,
                    `${mtt_detail.mtt.max_delay_apply_bl}`
                ]);
            } else {
                this.$Shangxian.children[1].getComponent(cc.Label).string = StringHelper.Format(i18nMgr.Get('MTT_State_DelayDetail'), [
                    `${mtt_detail.mtt.limit_total_buy_times}`,
                    `${mtt_detail.mtt.max_delay_apply_bl}`,
                    `${mtt_detail.mtt.addon_begin_bl}`,
                    `${mtt_detail.mtt.addon_end_bl}`
                ]);
            }
        } else {
        ...
        this.$CurrentMang.children[1].getComponent(cc.Label).string =
            `${mtt_detail.more.sb / 100}/${(mtt_detail.more.sb * 2) / 100}(${mtt_detail.more.ante / 100})`;
        //下一盲注
        ...
        this.$NextMang.children[1].getComponent(cc.Label).string =
            `${mtt_detail.more.nsb / 100}/${(mtt_detail.more.nsb * 2) / 100}(${mtt_detail.more.nante / 100})`;
        //涨盲时间
        this.$UpBlindTime.children[0].getComponent(cc.Label).string = `${i18nMgr.Get('MTT_State_UpBlindTime')}:`;
        this.$UpBlindTime.children[1].getComponent(cc.Label).string = StringHelper.Format(i18nMgr.Get('UITexasReport_Text_MatchZmsysj'), [
            `${mtt_detail.mtt.upblind_interval / 60}`
        ]);
        //记分牌(只显示最大记分牌)
