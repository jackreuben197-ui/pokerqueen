...
export default class UIMTTDetail_Player extends UIBasePlus {
    ...
    refreshTop(res?: any) {
        if (res) {
            ...
            this.$block1.children[2].getComponent(cc.Label).string =
                `${res.data.alive || 0}`;
            this.$block1.children[3].getComponent(cc.Label).string =
                `${res.data.total || 0}`;
        } else {
            this.$block2.active = this.mtt_detail.mtt.hunter_on == 1;
            this.$scroller_player.getComponent(cc.Widget).top =
                this.mtt_detail.mtt.hunter_on == 1 ? 534 : 362;
    ...
    click_hunter(items: cc.Node[], index: number) {
        ...
        items.forEach((item, index) => {
            ...
            this.setChildColor(
                item,
                "label",
                status ? TextColor.Color1 : TextColor.Color3,
            );
        });
        //////////////////////////////////
        switch (index) {
    ...
    //初始化滚动列表的补充数据
    private initEX() {
        this.hunter_group = new TabsGroup(
            this.$block2.children,
            this.click_hunter,
            this,
        );
